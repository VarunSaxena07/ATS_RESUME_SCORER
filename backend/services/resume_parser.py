...
def validate_file(file_data:bytes, filename:str)->Tuple[bool, str, Optional[str]]:
    ...
    file_type, detected_type = _detect_file_type(file_data, filename)
...
def _detect_file_type(file_data: bytes, filename: str) -> Tuple[Optional[str], str]:
    detected_type = _detect_mime_with_magic(file_data)
    if detected_type in SUPPORTED_MIME_TYPES:
        return SUPPORTED_MIME_TYPES[detected_type], detected_type

    if file_data.startswith(b'%PDF'):
        return 'pdf', detected_type or 'application/pdf'

    if _looks_like_docx(file_data):
        return 'docx', detected_type or 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'

    if file_data.startswith(b'\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1'):
        return 'doc', detected_type or 'application/msword'
    ...
    if extension_type:
        return extension_type, detected_type or Path(filename).suffix.lower()

    return None, detected_type or 'unknown'


def _detect_mime_with_magic(file_data: bytes) -> str:
    if magic is None:
        if _MAGIC_IMPORT_ERROR is not None:
            log_info(
                f'python-magic unavailable; using file signature fallback: {_MAGIC_IMPORT_ERROR}',
                context='resume_parser',
            )
        return ''

    try:
        return magic.from_buffer(file_data, mime=True) or ''
    except Exception as exc:
        log_info(
            f'libmagic MIME detection unavailable; using file signature fallback: {exc}',
            context='resume_parser',
        )
        return ''


def _looks_like_docx(file_data: bytes) -> bool:
